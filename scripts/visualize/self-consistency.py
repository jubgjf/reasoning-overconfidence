...
async def main():
    ...
    ece_dict = ece_by_groups(plot_df, "method", "recall")
    ...
    for ax, method_label, method in zip(axes, method_labels, method_order):
        ...
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
        ax.set_title(f"{method_short_name} (ECE={ece_dict[method]:.3f})", fontsize=10)
        ax.grid(True)
        ax.legend(fontsize=8)
    axes[0].set_ylabel("Recall")
    # plt.suptitle("Long-CoT vs Self-Consistency Calibration")
    plt.tight_layout(rect=(0, 0, 1, 0.95))
    plt.savefig(f"figures/self-consistency-qwen-{dataset}.pdf")
    plt.show()
