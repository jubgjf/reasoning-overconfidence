...
def extract_answer_and_logprob_confidence(
    ...
    if dataset_name in [DatasetName.GSM8K]:
        ...
        logprobs_unnormalized = dict(filter(lambda item: item[0] in "ABCD", logprobs_unnormalized_unfiltered.items()))
    else:
...
def extract_answer_and_confidence(
    ...
    dataset_name: DatasetName,
    question_turn: ResponsePerTurn,
    confidence_turn: ResponsePerTurn | None,
) -> Result[tuple[str, float], str]:
    postprocess_map = {
